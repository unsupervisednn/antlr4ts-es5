...
function () {
  ...
  (0, _createClass2["default"])(DefaultErrorStrategy, [{
    ...
    value: function sync(recognizer) {
      var s = recognizer.interpreter.atn.states[recognizer.state]; //		System.err.println("sync @ "+s.stateNumber+"="+s.getClass().getSimpleName());
      ...
      var la = tokens.LA(1); // try cheaper subset first; might get lucky. seems to shave a wee bit off
    ...
    value: function singleTokenInsertion(recognizer) {
      var currentSymbolType = recognizer.inputStream.LA(1); // if current token is consistent with what could come after current
      ...
      var expectingAtLL2 = atn.nextTokens(next, PredictionContext_1.PredictionContext.fromRuleContext(atn, recognizer.context)); //		console.warn("LT(2) set="+expectingAtLL2.toString(recognizer.getTokenNames()));
    ...
    value: function getErrorRecoverySet(recognizer) {
      ...
      recoverSet.remove(Token_1.Token.EPSILON); //		System.out.println("recover set "+recoverSet.toString(recognizer.getTokenNames()));
